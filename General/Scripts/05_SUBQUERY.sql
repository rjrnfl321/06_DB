...
-- 6. 스칼라 서브쿼리
--    (== SELECT절에 사용되는 단일행 서브쿼리) 결과로 1행만 반환
--    SQL에서 단일 값을 가르켜 '스칼라'라고 함

-- 1) 각 직원들이 속한 직급의 급여 평균 조회
SELECT JOB_CODE, AVG(SALARY)
FROM EMPLOYEE
GROUP BY JOB_CODE
GROUP BY JOB_CODE ASC;
...
-- 3) 각 직원의 이름, 직급코드, 직급코드 || '직급별 급여 평균' 조회
SELECT
	EMP_NAME,
	JOB_CODE,
	(SELECT AVG(SALARY)
...
-- 1) RANK() OVER :
--     OVER() 괄호에 작성되어 정렬 기준대로 정렬 후 순위 지정
