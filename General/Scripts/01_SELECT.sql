 ...
 * 
 * - CURRENT_DATE : 현재 세션 시간
 * 
 * - SYSTIMESTAMP : 시스템이 나태내고 있는 현재 시간
 * 										-> ms 단위 + 지역 포함
 * 
...
SELECT '2024-08-15', TO DATE('2024-08-15','YYYY-MM-DD')
...
--	EMPLOYEE 테이블에서
--	모든 사원의 사번, 이름, 입사일, 현재까지 근무 일수 조회
SELECT EMP_ID ,
			 EMP_NAME,
			 HIRE_DATE,
 ...
 * 
 * 2) 컬렴명 AS "별칭" : 문자 O, 띄어쓰기 O, 특수문자 O
 * 
 * - ORACLE 에서 ""의 의미
...
/* 연결 연산자 (||) 
 * - 두 컬럼을 이어서 하나의 컬럼으로 조히할 때 사용*/

SELECT EMP_ID ,EMP_NAME , -- 200, 선동일
				EMP_ID || EMP_NAME  -- 200 200선동일
FROM EMPLOYEE;
 ...
 * 		값이 존재하지 않는다는 (빈칸)을 의미
 * 
 * ***************************
 * 컬럼명 IS NULL / 컬럼명 IS NOT NULL (O)
 * 		-> 지정된 컬럼에 값이 존재 하지 않는 경우 / 하는 경우
 * 			 (값의 유무를 따짐)
 * ***************************
 * 
 * */
